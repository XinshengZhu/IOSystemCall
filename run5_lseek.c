...
void lseekFromFile(int fd, size_t block_size, size_t block_count) {
    off_t offset;

    for (size_t i = 0; i < block_count; i++) {
        offset = block_size;
        if (lseek(fd, offset, SEEK_CUR) == (off_t)-1) {
            perror("Failed to lseek in file");
            exit(EXIT_FAILURE);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    if (st.st_size < (off_t)(block_size * block_count)) {
        fprintf(stderr, "%lu bytes file size is not big enough for the specified %lu bytes lseek operation.\n", st.st_size, block_size * block_count);
        exit(EXIT_FAILURE);
    }
    ...
    printf("Lseek %zu blocks of size %zu bytes from file '%s'\n", block_count, block_size, filename);
    gettimeofday(&start_time, NULL);
    lseekFromFile(fd, block_size, block_count);
    gettimeofday(&end_time, NULL);
    elapsed_time = (end_time.tv_sec - start_time.tv_sec) + (end_time.tv_usec - start_time.tv_usec) / 1000000.0;
    performance = (block_size * block_count) / elapsed_time;
    printf("Lseek Elapsed time: %f seconds\n", elapsed_time);
    printf("Lseek Performance: %f B/s / %f MiB/s\n", performance, performance / 1024 / 1024);
